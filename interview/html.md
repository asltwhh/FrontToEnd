...
> - 一般浏览器访问服务器并发送第一次请求时，服务器会创建一个session对象，并且生成一个与之对应的sessionId(我的理解类似于一个键值对{sessionId:session对象})。客户端第二次发起请求时就会判断cookies中有没有当前请求域名下的信息，有则会携带。服务端会从 Cookie 中获取 sessionId，再根据 sessionId查找对应的 Session 信息，如果没有找到说明用户没有登录或者登录失效，如果找到 Session 证明用户已经登录可执行后面操作。
