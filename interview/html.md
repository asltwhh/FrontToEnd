...
>
> - ```
>   <meta charset="utf-8" />   // 指定文档所使用的的编码方式
>     
>   // 指定视口是完美视口，
...
> - cookie 是服务器发送到用户浏览器并保存在本地的一小块数据，它会在浏览器下次向同一服务器再发起请求时被携带并发送到服务器上。js中存在直接操作cookie的方法，使用document.cookie设置和读取。在express中使用cookie-parser包，使用res.cookie设置cookie信息，使用req.cookie读取cookie信息。
> - 一般浏览器访问服务器并发送第一次请求时，服务器会创建一个session对象，每个seesion对象都有一个唯一的sessionId。服务器会以自动回话cookie将sessionId返回到浏览器(客户)端，浏览器下次再访问时，携带sessionId，找到对应的session。客户端第二次发起请求时就会判断cookies中有没有当前请求域名下的信息，有则会携带。服务端会从 Cookie 中获取 sessionId，再根据 sessionId查找对应的 Session 信息，如果没有找到说明用户没有登录或者登录失效，如果找到 Session 证明用户已经登录可执行后面操作。
> - **sessionId连接cookie和session**
> - 区别：
>   - 安全性：Session 比 Cookie 安全，Session 是存储在服务器端的，Cookie 是存储在客户端的。所以cookie一般用来存放不敏感的信息，比如用户设置的网站主题等。另外，每次http请求都会携带cookie信息，如果cookie数据过大，会产生性能问题。session一般保存敏感信息，比如用户的登录信息。
>   - 存取值的类型不同：Cookie 只支持存字符串数据，想要设置其他类型的数据，需要将其转换成字符串，Session 可以存任意数据类型。
>   - 有效期不同：Cookie 可设置为长时间保持，比如我们经常使用的默认登录功能，Session 一般失效时间较短，客户端关闭（默认情况下）或者 Session 超时都会失效。
>   - 存储大小不同：单个 Cookie 保存的数据不能超过 4K，Session 可存储数据远高于 Cookie，但是当访问量过多，会占用过多的服务器资源。并且sessionId的大小肯定小于所有的结果

### 21 cookies，sessionStorage 和 localStorage 的区别？

> - webStorage包括：SessionStorage， LocalStorage
> - 相同点：
>   - 这三者都可以被用来在浏览器端存储数据，而且都是字符串类型的键值对。区别在于前两者属于 HTML5 WebStorage，创建它们的目的便于客户端存储数据。而 cookie 是网站为了标示用户身份而储存在用户本地终端上的数据（通常经过加密）。cookie 数据始终在同源（协议、主机、端口相同）的 http 请求中携带（即使不需要），会在浏览器和服务器间来回传递。
> - 不同点：
>   - 存储大小：
>     - cookie 数据大小不能超过4 k 。
...
> - 5 Html5 websocket 协议:浏览器一定版本要求，服务器需要支持 websocket 协议
> - 6 iframe+hash
> - 设置代理服务器
