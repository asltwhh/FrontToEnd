来源：https://wangdoc.com/javascript/bom/form.html#formdata-%E5%AF%B9%E8%B1%A1
...
> - 所有的键值对都会提交到服务器。但是，提交的数据格式跟`<form>`元素的`method`属性有关。该属性指定了提交数据的 HTTP 方法。
>   - 如果是 GET 方法，所有键值对会以 URL 的查询字符串形式，提交到服务器，比如`/handling-page?user_name=张三&user_passwd=123&submit_button=提交`,get请求没有请求体
...
`FormData()`首先是一个构造函数，用来生成表单的实例对象。
...
但是此时formData仍然是一个只包含__proto__属性的空对象，username等属性并不是直接放入该对象中的
```
...
```
var formData = new FormData();  // 创建一个空的表单
...
发现一个问题：只要定义了上面的样式并且指定了表单项需要满足的条件，加载页面后，会自动进行校验，并显示出对应的样式，在修改了内容后，样式也会改变
...
// 触发整个表单的校验，如果有一个控件不符合条件，则返回false
form.checkValidity()
...
在它确定之后，在http请求中相当于设置了请求头：`Content-Type`
...
for (var i = 0; i < files.length; i++) {
  ...
  // 对于同一个键photos,对应多个值file,每个file的名字就是file.name
