...
webpack是用Js语言编写的，使用了nodejs中的依赖包，即nodejs为其提供了运行环境，所以说webpack依赖于node这个运行环境
...
webpack中也使用了这些变量，并且使用了Nodejs的其他内置模块，比如path等
...
module.exports = {
    ...
    output:{
        ...
        // 上面两句话的意思：文件输出到webpack.config.js文件的目录/build/js/built.js
    },
    // loader配置
    module:{
        // 详细地loader配置
...
module: {
   rules:[
       ...
       // 2 less文件
       {
        // 设置匹配哪些文件，使用正则表达式，下面表示匹配以less结尾的文件
        test:/\.less$/,
        // 进行哪些loader处理
        use: [
            // user中loader执行顺序：从右至左 依次执行
...
- html-webpack-plugin 插件将为你生成一个 HTML5 文件， 在 body 中使用 `script` 标签引入你所有 webpack 生成的 bundle。 只需添加该插件到你的 webpack 配置中，
...
- 此时样式被内嵌到了出口文件built.js中，style-loader还未起作用，

  ![](./img/51.png)

- 注意：**style-loader产生的style标签并不会直接显示在打包后的html中，因为此时style-loader还没有起作用，当我们运行打包得到的index.html文件时，就可以看到在浏览器的开发工具下的元素的Head中出现了一个style标签**，即在浏览器中打开时，style-loader才会解析built.js中的样式模块，将其提取出来产生一个style标签，插入到html中
...
- 1 安装包：
  ...
  ```
  - 默认情况下，webpack 处理不了 img 图片，处理样式中的图片资源需要借助`url-loader`,而它又是依赖于`file-loader`产生作用的，所以首先就需要下载这两个 loader 包；另外，处理 html 中的图片需要使用'html-loader'先将图片引入到目标 js 文件中
  - html-loader: 将html(包括其中的图片资源)转换为字符串引入到目标js文件
...
- 使用 webpack 命令打包之后得到的结果为： 
  ![16](./img/16.png) 
- 当然我们可以自定义 css 文件的名字和位置：**此时js和css仍然属于同一个chunk**
...
core-js包：core-js 就是提供新语法api一个集合库

- **它是JavaScript标准库的polyfill**，它尽可能的进行模块化，让你能选择你需要的功能
- **它可以不污染全局空间**
- 它和babel高度集成，可以对core-js的引入进行最大程度的优化

core-js@3 特性概览

- 支持ECMAScript稳定功能(比如promise等新功能)，引入core-js@3冻结期间的新功能，比如flat
...
### 更好地理解babel/polifill，core-js和@babel-runtime
...
https://babel.docschina.org/docs/en/usage/#polyfill

babel-polyfill 

1. 它融合了 core-js 和 regenerator-runtime,因此'babel-polyfill' 本质就是'corejs',它用于模拟完整的ES2015+环境。 babel 的 polyfill 机制是，对于例如 Array.from 等静态方法，直接在 global.Array 上添加；对于例如 includes 等实例方法，直接在 global.Array.prototype 上添加。这样直接修改了全局变量的原型，有可能会带来意想不到的问题。这个问题在开发第三方库的时候尤其重要，因为我们开发的第三方库修改了全局变量，有可能和另一个也修改了全局变量的第三方库发生冲突，或者和使用我们的第三方库的使用者发生冲突。公认的较好的编程范式中，也不鼓励直接修改全局变量、全局变量原型。**污染了全局空间**
2. 安装时需要`--save`，不能添加`-dev`,因为它需要在源码之前运行，等于先将函数放入全局或者对象原型上，供代码使用
...
1. core-js的结构是高度模块化的，它把每个特性都组织到一个小模块里，然后再把这些小模块组合成一个大特性，层层组织。比如： core-js/es6（core-js/library/es6）就包含了全部的ES6特性，而core-js/es6/array（core-js/library/es6/array）则只包含ES6的Array特性，而core-js/fn/array/from（core-js/library/fn/array/from）则只有Array.from这个实现。实现按需使用，就是自己选择使用到的特性，然后导入即可。而且不会污染全局控件。https://www.jianshu.com/p/e9b94b2d52e2

2. regenerator-runtime作为 `core-js` 的拾遗补漏，主要是 `generator/yield` 和 `async/await` 两组的支持。当代码中有使用 `generators/async` 时自动引入。

@babel-runtime:

1. @babel/polyfill直接在代码中插入了帮助函数，引入文件的体积过大。为了解决部分兼容性问题，引入了所有的兼容性代码；可以使用core-js结合babel-loader配置实现按需加载      4.5

2. Babel为了解决这个问题，提供了单独的包babel-runtime用以提供编译模块的工具函数， 启用插件babel-plugin-transform-runtime后，Babel就会使用babel-runtime下的工具函数实现转换，体积小，不污染全局环境。babel-runtime不会为对象添加新方法，它只会引入新的方法，直接调用。**但是，`runtime` 也不是包治百病，babel-runtime不能转换实例方法**
...
在`./lib/myWebpack/index.js`中定义webpack,产生Compiler实例对象：
...
在`./config/webpack.config.js`中定义配置文件：
...
1. 将自定义的配置传递给webpack，根据自定义的配置产生对应的Compiler实例
2. 使用该实例开启打包
...
1. 首先需要获取到入口文件的内容，用到了同步的方法：readFileSync(filePath,utf-8),返回内容为文本形式的文件

2. 然后将该内容解析为ast抽象语法树，此过程用到了三个包

   1. @babel/parser:将文件中的数据解析为一个抽象语法树。第二个参数指定模块化的方式。sourceType: "module"表明解析文件的模块化方案是ES module。得到的抽象语法树实际上就是一个对象，program的body中保存着入口文件中每一行代码，每一行代码都对应一个Node,引入语句的类型时ImportDeclaration,其他语句的类型就是ExpressionStatement

      ![](./img/48.png)

   2. 收集依赖：@babel/traverse包，traverse会遍历上面的ast对象中的body的所有项。并且判断里面的语句类型，如果该Node的type是ImportDeclaration 就会触发当前对应的函数，获取该依赖文件的绝对路径(因为我们在ast对象中的body中保存的source都是相对路径)，并且将其加入到依赖对象中。最终得到的依赖对象的结果就是
   ...
   3. 编译代码：目前的代码还是es6代码，还需要将其解析为js格式。将代码中浏览器不能识别的语法进行编译。需要引入一个库：@babel/core，将ast抽象语法树中浏览器不能识别的语句格式编译为浏览器可以识别的commonJS格式的代码,转换后得到的是一个对象，通过解构赋值可以得到code属性就是commonJS格式的代码：
...
1. 在Webpack中只负责创建一个Compiler实例
2. 将babel干的事情放在模块parser中，该模块主要干一件事，定义三个函数：
   1. 函数1：getAst，将文件解析成为ast
   2. 函数2：getDeps，添加依赖
   3. 函数3：getCode，将ast解析成为浏览器可以识别的commonJS格式的内容，并且提取code部分

3. 在模块Compiler中，定义开启打包的方法，并且开始打包，在run方法中分别调用parser模块中定义的三个方法
...
由于在入口文件中所包含的一些其他的依赖文件，所以还需要打包它的依赖文件,...。所以考虑新建一个函数，专门负责解析成为ast、添加依赖和转换格式。这样在run方法中递归地去对于每一个模块完成以上的三个步骤，实现入口文件中所有依赖、及其依赖的依赖的解析。

1. 在build方法中定义解析成为ast、添加依赖和转换格式，得到每一个模块的
...
2. 在run方法中：从入口文件开始递归地调用build方法，保存所有依赖模块得到的结果，保存在Compiler实例的modules属性中
  ...
  generate(depsGraph) {
    ...
    (function(depsGraph){
        ...
        function require(module){
            ...
            // 执行下面的立即执行函数中，会根据code中的内容不断地修改exports对象，而依赖模块中可能还存在require引入了其他的模块，所以又会造成递归
    ...
    })(${JSON.stringify(depsGraph)})`;   // 需要转为JSON字符串
...
它只会遍历forEach方法调用之前数组中的元素，在forEach方法调用过程中数组内部加入的元素不会被遍历到

如果需要遍历到新增的元素就不能使用forEach方法
...
总结：在配置文件package.json中设置启动build模块的脚本`"build": "node ./script/build.js",`

1. 在build.js中首先创建一个compiler实例对象，传入的参数就是webpack.config.js中导出的设置对象
2. 然后执行run方法开启打包。打包会经历以下步骤：
   1. 解析入口文件：
      1. 将入口文件转为ast抽象语法树
      2. 然后将该对象中的body属性的每一个语句对应的Node对象通过判断语句类型，将类型为`ImportDeclaration`的Node对象加入到依赖容器deps中
      3. 编译入口文件中的代码，将其转换为CommonJS格式，得到code
      4. 将{filePath,deps,code}放入当前实例对象的modules属性中
   2. 外层循环遍历modules,内层循环遍历该模块的依赖容器，最终所有模块的解析结果全部放在modules中
   3. 重新调整modules中元素的格式。调整为更好的依赖关系图
3. 产生buddle,指定文件输出的位置，输出。
...
bundle由许多chunk组成，chunk有几种类型，比如说“入口”和“子块”。通常chunk和输出的bundle一一对应，但是，有些是一对多的关系。

### 8.2 webpack的工作原理

webpack是把项目当作一个整体，通过给定的一个主文件，webpack将从这个主文件开始找到你项目当中的所有依赖的文件，使用loaders将webpack本身不能识别的文件类型修改为它可以识别的类型，最后将它们打包输出成一个或多个浏览器可识别的js文件
...
解决图片的打包问题：
	...
	html-loader:将html(包括其中的图片资源)转换为字符串形式引入到目标js文件中

解决ES6语法不兼容问题：
	方法1：babel-loader：把 ES6 转换成 ES5，配合@babel/core @babel/preset-env使用
	方法2：babel-polifill   文件太大
	方法3：babel-loader结合core-js实现按需加载
...
```
html-webpack-plugin:实现html资源的打包，并且引入所有的打包输出的资源（js/css)

mini-css-extract-plugin:配合MiniCssExtractPlugin.loader,将样式文件从目标js文件中提取出来，打包为一个单独的文件，并且在html中引入
...
- 初始化参数：从配置文件`webpack.config.js`和 Shell 语句`webpack 可能有其他参数，比如环境等`中读取与合并参数，得出最终的配置对象；

- 开始编译：用上一步得到的配置对象初始化 Compiler 对象，加载所有配置的插件，执行对象的 run 方法开始执行编译；
