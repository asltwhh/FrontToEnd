...
1. react使用的是自定义的合成事件，即on+Xxx，而不是使用原生的DOM事件(on+xxx)，它将原生中的事件都重新写了一份，**主要是为了更好的兼容性**
...
>
>     ​					组件A订阅消息：`var this.token = PubSub.subscribe('search',(msg,paras)=>{})`，msg是消息名称，paras是发布消息时传入的参数。将消息的订阅保存在组件实例对象的token属性中，方便取消消息的订阅
>
...
>   ```
>
>   鉴于此，**只能在函数内部的最外层调用hook，不能在循环、条件判断或者子函数中调用，来确保hooks的执行顺序一致,否则会报错 React Hook "useState" is called conditionally. 。**
>
...
>   ```
>
>   第一，react首次渲染成功后会调用一遍传给useEffect的函数。
...
(2). 语法: 
	第一步：创建容器
		const refContainer = useRef();     // refContainer:{current:undefined}
	第二步：
        <input ref={refContainer} type='text' />    // refContainer:{current:input}
        function show(){
...
之前在一个组件中，引入一个子组件都是使用自闭合的结构：`<div> <A /> </div>`,并没有传入过包含有内容体的组件，例如`<div> <A>xxxx</A> </div>`,但是页面上展示不出`xxxx`。
...
>     - 在A组件中调用render函数，会返回组件B
>   - 第二步：在其他组件中定义A和B的父子关系,并且通过props传递数据给B组件，`<A render={(name) => <B name={name} />} />`
>     - 向A组件传递一个render属性，该属性的值是一个函数，函数的返回值是组件B，并且向B传递了数据
