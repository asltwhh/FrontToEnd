...
- `div > span`:选中div块中的所有子元素span(直接儿子)
...
#### 3 伪类及伪类选择器
描述一个元素的特殊状态：如第一个元素，被点击的元素，鼠标移入的元素
...
`<span style='color:red !important;'>哒哒</span>`
...
- 如果有一个值为auto,则浏览器会率先调整那个为auto的值
如果两个外边距均为auto,则会将其设置为相同的值，子元素在父元素中居中显示
- 如果超出了父元素宽度大于子元素内容区宽度，则会将子元素margin-right调整为负值
注意只有margin相关属性可以取值auto
...
- 可选值：
    ...
    - `box-sizing`:width和height指定盒子大小(内容区+内边距+边框)
...
- BFC是css中的隐函属性，开启BFC的元素会变成一个独立的布局区域
- 开启BFC后元素的特点
```
- 不会被浮动元素所覆盖
- **子元素和父元素外边距不会重叠**
...
- **如何开启BFC呢？**
```
- 给元素设置浮动(不推荐，浮动之后父元素的宽度会丢失)
- 将元素设置为行内块元素(不推荐)
- 将元素的overflow设置为hidden或者auto,一般设置为hidden(推荐使用)
```
...
- 使用`before伪元素`:在父元素的内部的最上面为其**添加一个伪元素before**,设置其内容为空，并为期增加`display:table;`属性(将元素转换为块级表格)，从而使用内容将父元素外边距和子元素外边距阻隔开，从而导致其**不满足相邻条件**
```
<div class="box1">
    <div class="box2"></div>  
</div>  
...
- 在相对定位中，默认偏移量是相对于定位元素自身而言的，指的是定位元素相对于它本身应该所在的位置的偏移量
...
```
- 在开启定位后，包含块指的是`离当前元素最近的开启了定位的祖先块元素`,如果所有祖先元素均没有开启定位，则包含块就是根元素html
...
- 将其设置为绝对定位，偏移量设置为0，则其就位于浏览器的左上角的位置，当我们向下滑动页面时，该块就会跟随页面向上移动；
```
- 很多的小广告就是使用绝对定位产生的
...
#### 1、行高
- 行高：文字占页面的实际高度，通过line-height设置
- 可以直接指定大小，也可以直接为行高设置一个整数(表示字体大小的整数倍)
...
- 对于图片而言，一般都是沿着基线对齐，所以在其底部和框线直线会存在间距，为了去除这个间距，一般会使用`vertical-align`,只要取值不是`baseline`就可以达到效果。
...
- 产生实心小三角：使用0宽度0高度的块元素的边框,可以发现当我们为其设置了较大的边框，并且每个边框的颜色不相同时就会出现四个明显的小三角，为了得到最下方的小三角，我们可以先将所有的边框颜色设置为透明`transparent`，然后单独将下方边框的颜色设置为非透明，这样我们就得到了一个尖尖向上的小三角box3:[小三角](练习/09.小米官网创建/小三角.html)
    ```
    <div class="box1"></div>  
    <div class="box2"></div>  
    <div class="box3"></div>  
...
- `animation-timing-function`:设置动画的时序函数,这与过渡中的效果一直
...
- `animation-direction`:设置动画的执行方向
    ...
    - `alternate-reverse`:从to到fram,当`animation-iteration-count`不为1时，重复执行from到to
...
#### 1、x和y轴的平移实现
- 使用它可以实现未指定宽度和高度的元素居中：[效果展示](练习/13.变形平移/02.居中练习.html)
```
...
- less语法大体上和css语法相同，但是增加了很多对css的扩展，所以浏览器无法直接执行less代码，必须先将less编译成css格式
...
- 像素：屏幕是由一个个发光的小点构成，每个小点就是像素
    - 像素越小，屏幕越清晰
    - 本电脑屏幕尺寸：13.9英寸 (31厘米X17厘米) 
- 分辨率：(本电脑)1366×768，指的就是屏幕中小点的数量
    - 可以得到本电脑：1厘米有1366/31=44个像素
- 在前端开发中，分为**CSS像素**和**物理像素**，分辨率指的就是物理像素
    - CSS像素：编写网页时所用的像素
    - 浏览器在显示网页时，需要将CSS像素转换为物理像素，然后再呈现
    - 一个CSS像素最终由几个物理像素显示，取决于浏览器
    - 默认情况下1个CSS像素===一个物理像素
- 视口：
    - 屏幕中用来显示网页的区域，通过最大化最小化可以改变视口大小
    - 默认情况下视口宽度为(本电脑最大化)1366px(CSS像素)，此时物理像素与CSS像素是1:1的
    - 在网页中查看html的宽度就是视口宽度
    - 当我们放大网页为200%，视口宽度：
        - CSS像素：675px
        - 物理像素：1366px(我们用截图工具得到的也是物理像素)
        - 此时两者比例：1:2
- 手机像素
    - 一般智能手机的像素远远小于电脑显示屏的像素，分辨率相对高一些
    - 默认情况下，移动端的网页均会将视口设置为980px(CSS像素),从而确保PC端的网页可以在移动端正常显示，但是如果PC端网页宽度超过了980px,则移动端的浏览器会自动对其进行缩放从而保证网页的完整显示
    - 但是往往用户效果不是很好，所以大部分网站都会再设计一个移动端网页
- 完美视口
    - 移动端默认的视口大小是980px(CSS像素)
    - 默认情况下，移动端的像素比是  980px/移动端宽度(物理像素)
        - 对应于iphone6而言，就是980/750  (可以在`https://uiiiuiii.com/screen/`查看)
    - 如果直接在网页中编写移动端代码，这样在980px的视口下像素比是非常小的，从而导致网页中的内容非常小
    - 所以我们在编写移动端页面时，需要确保有一个合理的像素比，所以会另外设置移动端的视口大小
        - 一般：一个css像素  对应  2个物理像素
        - 或者：1个css像素   对应  3个物理像素
    - 可以通过meta改变网页的视口大小
        - `<meta name='viewport' content='width=375px;'>`
        - 此时视口大小变成了100px,则像素比就是375/750=1:2
        - 每一款移动设备在设计时均会有一个最佳的像素比，一般我们只需要将像素比设置为最佳值，即可得到最佳效果，`对于iphone6而言是1:2，则视口宽度就应该设置为375px`
    - 但是我们设计时不能只考虑一台移动设备，需要考虑多个型号的设备，所以一般需要`device-width`这个变量，设置为`<meta name='viewport' content='width=device-width;'>`,这样就可以在所有移动设备上正常显示了。
        - 对于iphone6而言，最佳像素比2,物理像素为750px,决定了device-width=350px
        - 对于iphone6plus,最佳像素比为2.6，物理像素为1080px,从而决定了device-width为414px
- `vw`单位
    - 不同设备的完美视口大小也不相同
        - iphone6: 375px
        - iphone6plus: 414px
    - 由于不同设备的视口和像素比不同，所以元素在不同的设备中的表现不同，例如一个宽度为375px的元素
        - 在iphone6中，像素比是1:2，对应于物理像素就是750px,从而该元素显示时是占满全屏幕的
        - 在iphone6plus，像素比是1:2.6，对应于物理像素就是975px,而该设备的物理像素是1080px,所以该元素在显示时不是占满屏幕的
        - 这就导致了相同页面在不同设备中显示不同，比如在iphone6中三个元素占满了整个屏幕，而在iphone6plus中旁边还有留白，就不美观了
    - 所以在移动端的设计中就不能使用像素作为单位制定元素的宽度和高度
    - 使用`vw`作为单位：`viewport`
        - 表示视口宽度
        - 
        - 指定元素宽度`width:100vw;`表示一个视口宽度
            - 在iphone6中就是375px
            - 在iphone6plus中就是414px
        - 可以放心在移动端使用
    - 比如对于一个设计图：750×1150px 
        - 想要显示一个宽度为35px(css像素)的一个元素，即需要保证在不同设备中显示的比例相同，就可以指定该元素的宽度为100vw/750px*35px
        - 100vw/750px:指定一个像素包含了多少vw
        - 100vw/750px*35px:指定该元素包含了多少个vw
- `vw适配`
    - 为了计算方便提出的
    - `1rem`：表示html的字体大小
    - `1em`:表示元素自身字体大小
    - 网页中字体最小为12px,其次是0，如果设置了小于12px,但是大于0的字体大小，会自动跳转为12px显示
```
html {
    font-size: 100vw/750px*10;
    <!-- 给它扩大10倍，否则字体大小小于12px -->
}
div {
    <!-- 然后设置元素宽度和高度，缩小10倍 -->
    width: 4.8rem;
...
```
- 例子参见：[简易移动页面练习](练习/16.像素/03.简易移动端页面练习.html)
    - 主要关注css的写法，使用的是less编写自动产生的css文件
